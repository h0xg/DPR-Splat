...
def adjust_intrinsics(gt_intrinsics, original_size, new_size):
    """
    Adjusts the camera intrinsic matrix to match a new image resolution.

    Args:
        gt_intrinsics (list or numpy.ndarray): Original intrinsic parameters as a flattened list.
        original_size (tuple): Original image size (width, height).
        new_size (tuple): New image size (width, height).

    Returns:
        numpy.ndarray: Adjusted 3x3 intrinsic matrix.
    """
    # Construct the intrinsic matrix from the provided parameters
    K = np.array([
        [float(gt_intrinsics[2]), 0, float(gt_intrinsics[4])],
        [0, float(gt_intrinsics[3]), float(gt_intrinsics[5])],
        [0, 0, 1]
    ], dtype=np.float32)

    W_o, H_o = original_size
    W_n, H_n = new_size
...
def denormalize_intrinsics(normalized_intrinsics, image_size):
    """
    Converts a normalized intrinsic matrix to the original intrinsic matrix based on image dimensions.

    Args:
        normalized_intrinsics (numpy.ndarray): Normalized 3x3 intrinsic matrix (values in [0, 1]).
        image_size (tuple): Image size (width, height).

    Returns:
        numpy.ndarray: Original 3x3 intrinsic matrix.
    """
    W, H = image_size

    # Scale the normalized intrinsic matrix to the original image dimensions
    original_intrinsics = normalized_intrinsics.copy()
    original_intrinsics[0, 0] *= W  # fx
    original_intrinsics[1, 1] *= H  # fy
    original_intrinsics[0, 2] *= W  # cx
    original_intrinsics[1, 2] *= H  # cy
