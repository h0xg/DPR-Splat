...
def save_gaussians(gaussians, name, iteration, final=False):
    if name is None:
        return
    if final:
        point_cloud_path = os.path.join(name,"gaussian_output","gaussian")
    else:
        point_cloud_path = os.path.join(
            name, "point_cloud/iteration_{}".format(str(iteration))
        )
    log_path = os.path.join(point_cloud_path, f"{iteration}.ply")
    os.makedirs(os.path.dirname(log_path), exist_ok=True)  # Ensure directory exists
    Log(f"save to {log_path}")
    gaussians.save_ply(log_path)