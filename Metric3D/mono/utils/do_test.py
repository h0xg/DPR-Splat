...
def do_scalecano_test_with_custom_data2(
    model: torch.nn.Module,
    cfg: dict,
    test_data: list,
    logger: logging.RootLogger,
    ...
    show_dir = cfg.show_dir
    save_interval = 1
    save_depth_dir = show_dir + '/depth'
    os.makedirs(save_depth_dir, exist_ok=True)
    save_normal_dir = show_dir + '/normal'
...
#     is_distributed: bool = True,
#     local_rank: int = 0,
#     bs: int = 1,  # Batch size parameter
# ):

#     show_dir = cfg.show_dir
#     save_interval = 1
#     save_depth_dir = show_dir + '/estimated_depth'
#     os.makedirs(save_depth_dir, exist_ok=True)
#     save_normal_dir = show_dir + '/estimated_normal'
#     os.makedirs(save_normal_dir, exist_ok=True)
...
#         rgb_origin = cv2.imread(test_data[i])[:, :, ::-1].copy()
#         if intrinsic is None:
#                 intrinsic = [1000.0, 1000.0, rgb_origin.shape[1]/2, rgb_origin.shape[0]/2]
#         rgb_input, _, pad, scale_info = transform_test_data_scalecano(rgb_origin, intrinsic, cfg.data_basic)
...
#         pred_depth_numpy = pred_depth.detach().cpu().numpy()

#         np.save("pred_depth.npy", pred_depth_numpy)
...
#             gt_normal = None
#             pred_normal_numpy = pred_normal.detach().cpu().numpy()

#             # # Step 2: 保存为文件 (以 .npy 为例)
#             np.save("pred_normal.npy", pred_normal_numpy)
...
def postprocess_per_image2(i, pred_depth, gt_depth, intrinsic, rgb_origin, normal_out, pad, an, dam, dam_median, dam_global, is_distributed, save_depth_dir, save_normal_dir, normalize_scale, scale_info):
    ...
    # pcd
    pred_depth = pred_depth.detach().cpu().numpy()
    np.save(osp.join(save_depth_dir,'depth_'+an['filename'].rsplit('.', 1)[0]),pred_depth)
    print("finish save",osp.join(save_depth_dir,'depth_'+an['filename'].rsplit('.', 1)[0]))
    #pcd = reconstruct_pcd(pred_depth, intrinsic[0], intrinsic[1], intrinsic[2], intrinsic[3])
    #os.makedirs(osp.join(save_pcd_dir, an['folder']), exist_ok=True)
    #save_point_cloud(pcd.reshape((-1, 3)), rgb_origin.reshape(-1, 3), osp.join(save_pcd_dir, an['folder'], an['filename'][:-4]+'.ply'))
    ...
    if normal_out is not None:
        ...
        np.save(osp.join(save_normal_dir, 'normal_'+an['filename'].rsplit('.', 1)[0]),pred_normal)
