...
import copy
import cv2
from ..utils.misc import mkdir_for, hash_md5
from ..cloud_opt.utils.losses import gamma_loss
from ..cloud_opt.utils.schedules import linear_schedule, cosine_schedule
from ..fast_nn import fast_reciprocal_NNs, merge_corres
...
def sparse_global_alignment(imgs, pairs_in, cache_path, model, subsample=8, desc_conf='desc_conf',
                            device='cuda', dtype=torch.float32, shared_intrinsics=False, init_K = None,mask_path= None,
                            original_sizes = None,save_mask =True,**kw):
    ...
    pairs, cache_path = forward_mast3r(pairs_in, model,
                                       ...
                                       desc_conf=desc_conf, device=device,mask_path = mask_path,original_sizes = original_sizes,save_mask=save_mask)
    ...
    imgs, res_coarse, res_fine = sparse_scene_optimizer(
        ...
        shared_intrinsics=shared_intrinsics, cache_path=cache_path, device=device, dtype=dtype,init_K = init_K ,**kw)
...
def sparse_scene_optimizer(imgs, subsample, imsizes, pps, base_focals, core_depth, anchors, corres, corres2d,
                           ...
                           shared_intrinsics=False,init_K = None,
    ...
    else:  #if we have gt k ,we freeze it
        if init_K is not None:
            pps = [nn.Parameter(pp.to(dtype), requires_grad=False) for pp in pps]
            log_focals = [nn.Parameter(f.view(1).log().to(dtype), requires_grad=False) for f in base_focals]
        else:
...
def create_expanded_mask(image_shape, matche0, square_size=9):
    """
    Create a mask by expanding pixel points into a square region.

    Args:
        image_shape (tuple): Shape of the target image (height, width).
        matche0 (list or ndarray): List of pixel coordinates [(x1, y1), (x2, y2), ...].
        square_size (int): Size of the square to expand each point (must be odd).

    Returns:
        mask (ndarray): Binary mask of the expanded regions.
    """
    # Initialize an empty mask
    mask = np.zeros(image_shape, dtype=np.uint8)
    
    # Half size of the square for expansion
    ...
    for x, y in matche0:
        ...
        x_max = min(image_shape[1], x + half_size + 1)
        y_min = max(0, y - half_size)
        y_max = min(image_shape[0], y + half_size + 1)
...
def forward_mast3r(pairs, model, cache_path, desc_conf='desc_conf',
                   device='cuda', subsample=8, mask_path = None,original_sizes = None,save_mask = True ,**matching_kw):
    res_paths = {}
    for idx, (img1, img2) in enumerate(tqdm(pairs)):
        ...
        if not all(os.path.isfile(p) for p in (path1, path2, path_corres)):
            ...
            if save_mask:
                if idx==0:
                    mask =  np.ones(original_sizes, dtype=np.int32)
                    file_name = f"mask_{idx}.npy"
                    np.save(os.path.join(mask_path,file_name), mask)
                    mask = create_expanded_mask(img1['true_shape'][0],corres[1])
                    mask = cv2.resize(mask, original_sizes, interpolation=cv2.INTER_NEAREST)
                    file_name = f"mask_{idx+1}.npy"
                    np.save(os.path.join(mask_path,file_name), mask)
...
def extract_correspondences(feats, qonfs, subsample=8, device=None, ptmap_key='pred_desc'):
    ...
    # np.save("/home/lingxiang/SLAM/MonoGS/ssss/idx1.npy",np.array(idx1,dtype=object))
    # np.save("/home/lingxiang/SLAM/MonoGS/ssss/idx2.npy",np.array(idx2,dtype=object))
