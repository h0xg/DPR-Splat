...
from dust3r_utils import  compute_global_alignment, loaddust_images, storePly, save_colmap_cameras, save_colmap_images

import matplotlib.pyplot as pl

def list_files(directory, file_extension=None):
    """
    Generate a list of files in a directory.

    Args:
        directory (str): Path to the directory.
        file_extension (str, optional): Filter by file extension (e.g., ".jpg", ".png"). Default is None (all files).
...
def _convert_scene_output_to_glb(intrinsics,ori_size,filelist,outfile, imgs, pts3d, mask, focals, cams2world, cam_size=0.05,
    ...
    if as_pointcloud:
        print(filelist)
        # Example usage
        # directory_path = "/home/lingxiang/SLAM/vggsfm/examples/replica/images"
        # train_img_list = list_files(directory_path)

        train_img_list = filelist
        pts = np.concatenate([p[m.ravel()] for p, m in zip(pts3d, mask)]).reshape(-1, 3)
        col = np.concatenate([p[m] for p, m in zip(imgs, mask)]).reshape(-1, 3)
        color_4_3dgs = (col * 255.0).astype(np.uint8)

        #save to ply format
        output_colmap_path = "output"





        storePly(os.path.join(output_colmap_path, "points3D.ply"), pts, color_4_3dgs)
        ...
        save_colmap_images(cams2world, os.path.join(output_colmap_path, 'images.txt'), train_img_list)
    ...
    return outfile
def reshape_and_save_depthmaps(depthmap_list, h, w, output_dir):
    """
    ...
    for i, depthmap in enumerate(depthmap_list):
        # Check if the size matches h * w
        # if depthmap.size != h * w:
        #     print(f"Warning: Depthmap {i} size {depthmap.size} does not match ({h} * {w}). Skipping.")
        #     continue
        
        # Reshape to (h, w)
        reshaped_depthmap = depthmap.reshape(h, w)
        
        # Save as a .npy file
        output_path = os.path.join(output_dir, f"depthmap_{i:04d}.npy")
        np.save(output_path, reshaped_depthmap)
        print(f"Saved reshaped depthmap {i} to {output_path}")



def save_conf(conf_list, output_dir):
    """
    Reshape each depthmap in the list to (h, w) and save it to the output directory.
    ...
    for i, conf in enumerate(conf_list):
        ...
        np.save(output_path, conf)
        print(f"Saved reshaped depthmap {i} to {output_path}")



def get_3D_model_from_scene(ori_size,filelist,silent, scene_state, min_conf_thr=2, as_pointcloud=False, mask_sky=False,
    ...
    h,w,_ = rgbimg[0].shape


    # 3D pointcloud from depthmap, poses and intrinsics
    if TSDF_thresh > 0:
        tsdf = TSDFPostProcess(scene, TSDF_thresh=TSDF_thresh)
        pts3d, depthmap, confs = to_numpy(tsdf.get_dense_pts3d(clean_depth=clean_depth))
    else:
        pts3d, depthmap, confs = to_numpy(scene.get_dense_pts3d(clean_depth=clean_depth))


    output_dir = "output"
    print(h,w)
    print(depthmap[0].shape)
    print("confs",confs[0].shape)

    reshape_and_save_depthmaps(depthmap, h, w, output_dir)

    save_conf(confs, output_dir)
    msk = to_numpy([c > min_conf_thr for c in confs])
    return _convert_scene_output_to_glb(intrinsics,ori_size,filelist,outfile, rgbimg, pts3d, msk, focals, cams2world, as_pointcloud=as_pointcloud,
...
def get_reconstructed_scene(outdir, gradio_delete_cache, model, device, silent, image_size, current_scene_state,
    ...
    # print(filelist)
    # directory_path = "/home/lingxiang/SLAM/vggsfm/examples/replica/images"
    # filelist = list_files(directory_path)


    imgs,ori_size = load_images(filelist, size=image_size, verbose=not silent)
    if len(imgs) == 1:
    ...
    outfile = get_3D_model_from_scene(ori_size,filelist,silent, scene_state, min_conf_thr, as_pointcloud, mask_sky,
