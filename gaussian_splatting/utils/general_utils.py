...
def build_rotation(r):
    ...
    R[:, 2, 1] = 2 * (y * z + r * x)
    R[:, 2, 2] = 1 - 2 * (x * x + y * y)
    return R  #通过4元数构造旋转矩阵 转换方程 每个高斯都是这么处理


def build_scaling_rotation(s, r):
    L = torch.zeros((s.shape[0], 3, 3), dtype=torch.float, device="cuda")  #s.shape[0] total number of gaussian
    R = build_rotation(r)

    L[:, 0, 0] = s[:, 0] #缩放矩阵放到旋转矩阵里面
    L[:, 1, 1] = s[:, 1]
    L[:, 2, 2] = s[:, 2]

    L = R @ L
    return L #s是尺度，r是旋转 这个L是高斯球的变换
