...
class GaussianModel:
    def __init__(self, sh_degree: int, config=None):
        ...
        self.max_sh_degree = sh_degree  #球谐函数的阶  

        self._xyz = torch.empty(0, device="cuda")#高斯球的位置
        ...
        self.max_radii2D = torch.empty(0, device="cuda") #投影到平面之后二维高斯半径
        ...
        self.opacity_activation = torch.sigmoid #透明度使用的是sigmoid
    ...
    def build_covariance_from_scaling_rotation(
        ...
        L = build_scaling_rotation(scaling_modifier * scaling, rotation) #scaling factor rotator factor
        actual_covariance = L @ L.transpose(1, 2)
        symm = strip_symmetric(actual_covariance)   #只保留对角线上的R节省空间
        return symm

    @property
    def get_scaling(self):
        return self.scaling_activation(self._scaling) #激活函数的方法
    ...
    def create_pcd_from_image(self, cam_info, init=False, scale=2.0, depthmap=None): #从点云文件中创建
        cam = cam_info
    ...
    def create_pcd_from_image_and_depth(self, cam, rgb, depth, init=False):
        ...
        fused_color = RGB2SH(torch.from_numpy(np.asarray(pcd.colors)).float().cuda()) #球谐函数
        features = (
    ...
    def add_points(self, pcd: o3d.geometry.PointCloud, kf_id,global_scale_init=True):
